        ...
        if (data.length > 1) {
          ...
          for (let c = 0; c < row2.length; c++) {
            ...
            if (normalized === 'razonsocial') {
              ...
              uc12mmColIndex = c;
            } else if (normalized === '#cliente' || normalized === 'nºcliente' || normalized === 'numerocliente') {
              clientNumberColIndex = c;
            }
          }
          ...
          if (razonSocialColIndex !== -1) {
            ...
            for (let i = 2; i < data.length; i++) {
              ...
              // Provide a fallback to index 0 if not explicitly found but column headers are weird
              const cNumValue = clientNumberColIndex !== -1 ? data[i][clientNumberColIndex] : data[i][0];

              if (razonSocial && typeof razonSocial === 'string' && razonSocial.trim() !== '') {
                newClients.push({
                  id: `c${Date.now()}-${i}`,
                  clientNumber: cNumValue ? String(cNumValue).trim() : '',
      ...
      <div className="flex flex-col h-screen bg-slate-50">
        <header className="bg-slate-800 text-white px-4 py-3 flex justify-between items-center sticky top-0 z-10 shadow-md">
          <div className="flex items-center gap-2">
            <img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23B59A3A' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpath d='M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5'/%3E%3C/svg%3E" alt="SyncOps Logo" className="h-4 w-auto object-contain" />
            <span className="font-orbitron font-black uppercase tracking-tighter text-sm leading-none">Sync<span className="text-slate-400 font-medium mx-[1px]">·</span><span className="text-[#5F6D4F]">Ops</span></span>
          </div>
      ...
      <aside className={cn(
        ...
        <div className="p-6 flex items-center justify-between text-white">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-base font-orbitron font-black uppercase tracking-tighter leading-none">Sync<span className="text-slate-400 font-medium mx-[1px]">·</span><span className="text-[#5F6D4F]">Ops</span></span>
          </div>
